...
const ResturantComponent = ({ name, deliveryFee, address, imagesrc , ResturantId }) => {
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-pink-100 rounded-2xl shadow-md w-72 hover:shadow-lg transition">
      ...
      {/* Action Button */}
      <button onClick={() => navigate(`/SelectedResturantPage/${ResturantId}`)} className="mt-4 px-4 py-2 bg-pink-500 text-white rounded-lg hover:bg-pink-600 transition">
        مشاهده رستوران
      </button>
    </div>
  );
